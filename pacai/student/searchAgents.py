...
class CornersProblem(SearchProblem):
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                # corners [bottomLeft, topLeft, bottomRight, topRight]: same format as self.corners
                ...
                nexState = ((nextx, nexty), tuple(corners))
                successors.append([nexState, action, 1])
        # state = [(x,y), [0,0,0,0]]
        return successors
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners = list(problem.corners)
    # for u in problem.corners:
    #     unvisited_corners.append(u)
    # holds the nexr position
    nextPos = I_am_here
    ...
    while(unvisited_corners):
        ...
        d = []
        # holds the min distance
        m = 50000000000

        for corner in unvisited_corners:
            man = distance.manhattan(I_am_here, corner)
            d.append(man)
            if man == min(d):
                nextPos = corner
                m = man
        ...
        # m_index = d.index(m)
        # move to that corner
        # I_am_here = unvisited_corners[m_index]

        # marking the corner as visited
        unvisited_corners.remove(nextPos)
...
def foodHeuristic(state, problem):
    ...
    for f in foodGrid.asList():
        d = distance.maze(state[0], f, problem.startingGameState)
        dist.append(d)
